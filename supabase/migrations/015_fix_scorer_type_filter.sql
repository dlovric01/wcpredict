-- ============================================================
-- Migration 015: Scorer check — also exclude shootout_kick type
-- ============================================================
-- poll_live_matches now stores penalty shootout kicks as type='shootout_kick'
-- (detected via comments="Penalty Shootout" from the API).
-- The scoring function already uses minute <= 90 which excludes them, but
-- also explicitly filter type = 'goal' for correctness.
-- No functional change for matches already scored; only affects future scoring.
-- ============================================================
...
begin
  ...
  update public.predictions pr set
    ...
    -- Goalscorer: player scored ≥1 non-own-goal in regular time.
    -- type = 'goal' excludes shootout_kick rows (stored separately since migration 015).
    -- minute <= 90 excludes ET goals (API stores stoppage time goals at elapsed=90,
    -- ET goals at elapsed >= 91).
    points_scorer = case
